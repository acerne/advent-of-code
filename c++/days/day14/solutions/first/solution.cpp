...
public:
  ...
  void run(std::string polymer_template, uint8_t n_insertions) {
    ...
    for (int i = 0; i < polymer_template.size() - 1; i++) {
      ...
      const auto counts = search_node(
        Node{.a = polymer_template.at(i), .b = polymer_template.at(i + 1), .depth = n_insertions});
  ...
  size_t most_common_count() const {
    const auto comparison = [](auto& l, auto& r) -> bool { return l.second < r.second; };
    const auto max = std::max_element(m_char_counters.begin(), m_char_counters.end(), comparison);
    return max->second;
  }

  size_t least_common_count() const {
    const auto comparison = [](auto& l, auto& r) -> bool { return l.second < r.second; };
    const auto min = std::min_element(m_char_counters.begin(), m_char_counters.end(), comparison);
    return min->second;
  }
...
private:
  std::map<char, size_t> search_node(const Node& idx) {
    ...
    if (m_node_cache.count(idx) > 0) { // node exists, return cached counts
      ...
      char x = m_insertion_rules.at({idx.a, idx.b});
      return std::map<char, size_t>{{x, 1}};
    }
  }
...
Input Solution::read(const std::string& file_path) {
  ...
  while (std::getline(line_stream, line)) {
    inputs.insertion_rules.emplace(std::pair<char, char>{line.at(0), line.at(1)}, line.at(6));
  }
  return inputs;
}
