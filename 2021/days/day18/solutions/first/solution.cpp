...
protected: // Methods
  bool explode() {
    for (auto it = m_snailfish.begin(), prev = m_snailfish.end(); it != m_snailfish.end();) {
      if (it->depth > 4) {
...
private: // Methods
  ...
  std::optional<int> recursive_parser(const std::string& input, int depth) {
    ...
    if (left_value) {
      m_snailfish.push_back(Node{.value = *left_value, .depth = depth, .pos = Node::Pos::LEFT});
    }
    auto right_value = recursive_parser(right, depth + 1);
    if (right_value) {
      m_snailfish.push_back(Node{.value = *right_value, .depth = depth, .pos = Node::Pos::RIGHT});
    }
    return {};
  }
